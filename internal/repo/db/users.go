...
func (r *Repo) GetAdminUsers(ctx context.Context, adminGroups string) ([]domain.User, error) {
	var users []domain.User
	iter := r.FS.Collection("users").Where("groups", "array-contains", adminGroups).Documents(ctx)
	for {
		doc, err := iter.Next()
		if err == iterator.Done {
			break
